...
{
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 5, self.view.frame.size.width, KHeightS-5) style:UITableViewStyleGrouped];
        _tableView.sectionIndexColor= HWColor(186, 186, 186);
...
//  A  B  C  D
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(10, 15, 100, 22)];
...
- (NSArray *)sortRecords:(NSArray *)recordArray
{
    ...
    if (_objectComparisonStringBlock) {
        for (NSDictionary *object in recordArray) {
   
            //getUserName是实现中文拼音检索的核心，见NameIndex类
            NSString *objStr = _objectComparisonStringBlock([object objectForKey:@"nickname"]);
