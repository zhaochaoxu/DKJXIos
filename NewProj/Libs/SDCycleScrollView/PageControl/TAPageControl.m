...
{
    return CGSizeMake((self.dotSize.width + self.spacingBetweenDots) * pageCount - self.spacingBetweenDots + 10.0, self.dotSize.height);
}
...
{
    BOOL isSeletcedIndex = (_currentPage == index);
    if (isSeletcedIndex) {
        CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2) - 5;
        CGFloat y = (CGRectGetHeight(self.frame) - self.dotSize.height) / 2;
        
        dot.frame = CGRectMake(x, y, self.dotSize.width + 10, self.dotSize.height);
    }else{
