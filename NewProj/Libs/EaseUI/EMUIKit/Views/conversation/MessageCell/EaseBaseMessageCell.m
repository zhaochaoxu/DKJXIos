...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if (model.message.chatType == EMChatTypeGroupChat) {
        NSString *family_from_name = [ext objectForKey:@"family_from_name"];
        NSString *family_from_icon = [ext objectForKey:@"family_from_icon"];
        NSString *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@",family_from_icon]].absoluteString;
        _nameLabel.text = family_from_name;
        [self.avatarView af_setImageWithURL:[NSURL URLWithString:url] placeholderImage:model.avatarImage];

        
    }else if (model.message.chatType == EMChatTypeChat){
        if (ext) {
            NSString *uid = [RYUserInfo uid];
            if (![uid isEqual:[ext objectForKey:@"family_from_id"]]) {
                NSDictionary *dict = [[NSUserDefaults standardUserDefaults] valueForKey:[ext objectForKey:@"family_from_id"]];
                NSString *family_from_icon = [dict objectForKey:@"family_to_icon"];
                NSString *family_from_name = [dict objectForKey:@"family_to_name"];
                NSString *urlstring = [NSURL URLWithString:family_from_icon].absoluteString;
                
//                _nameLabel.text = family_from_name;
                [self.avatarView af_setImageWithURL:[NSURL URLWithString:urlstring] placeholderImage:model.avatarImage];
            }else{
                NSString *icon = [RYUserInfo userIcon];
                NSString *nickname = [RYUserInfo nickname];
//                _nameLabel.text = nickname;
                [self.avatarView af_setImageWithURL:[NSURL URLWithString:icon] placeholderImage:model.avatarImage];
            }
        }
