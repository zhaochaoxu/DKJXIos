...
- (void)didSendText:(NSString *)text
{
    if (text && text.length > 0) {
        //进行过滤
        NSString *filterText = [RYUtils filteringSensitiveWords:text];
        [self sendTextMessage:filterText];
...
- (void)_sendMessage:(EMMessage *)message
{
    ...
    if (dict !=nil) {
        [dict addEntriesFromDictionary:self.extDict];
        message.ext= dict;
        NSLog(@"meassage=%@",message.ext);
    }else{
