...
+ (NSString *)ry_formattedTimeFromTimeInterval:(long long)time{
    NSDate *oldDate = [NSDate dateWithTimeIntervalInMilliSecondSince1970:time];
    NSInteger offset = [[NSDate date] timeIntervalSinceDate:oldDate];
    if (offset < 60) {
        return [NSString stringWithFormat:@"%@秒前",@(offset).stringValue];
    }else if (offset < 60 * 60){
        return [NSString stringWithFormat:@"%@分钟前",@(offset/60).stringValue];
    }else if (offset < 60 * 60 * 24){
        return [NSString stringWithFormat:@"%@小时前",@(offset/60/60).stringValue];
    }
    NSDateFormatter* formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"YYYY-MM-dd"];
    return [formatter stringFromDate:oldDate];
}
